...
<template>
	<view class="date" :class="{'brief': brief || retract}">
		<view class="head" v-if="!brief">
			<view class="icon" @click="switch_month_week('prev',true)"><uni-icons type="left" size="20" /></view>
			<view class="title">{{nowYear+'年'+nowMonth+'月'}}</view>
			<view class="icon" @click="switch_month_week('next',true)"><uni-icons type="right" size="20" /></view>
		</view>
		<view class="box">
			<view class="clue" v-if="brief">
				<view class="year">{{nowYear+'年'}}</view>
				<view class="month"><span>{{nowMonth}}</span>月</view>
			</view>
			<view class="letter">
				<view class="date_dl" >
					<view class="dd" :class="{'nowadays': tickWeek === index}" v-for="(item,index) in week" :key="index">{{tickWeek === index ? '今' : item}}</view>
					<!-- <view class="dd" :class="{'nowadays': tickWeek === index}" v-for="(item,index) in week" :key="index">{{item}}</view> -->
				</view>
				<!-- <swiper :style="{height:(retract ? (brief ? 1 * 80 : 2 * 80) : (week_list.length + 1) * 80 ) + 'rpx'}" :current="current" circular @change="change_date"> -->
				<swiper :style="{height:(retract ? (brief ? 1 * 90 : 2 * 90) : (week_list.length + 1) * 90 ) + 'rpx'}" :current="current" circular @change="change_date">
					<swiper-item>
						...
						<view class="date_dl" v-show="!retract || index == to_prev_week_index" v-for="(item,index) in   week_list_prev_co" :key="index">
							<view class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
								<view class="num" :class="{'today': vo.today, 'month': vo.type == 'month', 'disabled': vo.type != 'month' && !retract, 'nowadays': vo.date.replace(/-(\d)(?!\d)/g, '-0$1') === nowadays}">{{vo.day}}</view>
								<view v-if="date_tip[vo.date.replace(/-(\d)(?!\d)/g, '-0$1')]" :class="{'tip': true, 'today': vo.today}">{{date_tip[vo.date.replace(/-(\d)(?!\d)/g, '-0$1')]}}</view>
								<!-- <view v-show="vo.dot && (vo.type == 'month' || retract)" class="dot"></view> -->
							</view>
						</view>
						<!-- <view @click="open" class="retract icon" v-if="!brief"><text class="iconfont next icon-fanhui" :class="[retract ? '' : 'retract_icon']" /></view> -->
					</swiper-item>
					<swiper-item>
...
export default {
	...
	methods:{
		...
		updateNowadays() {
			this.nowadays = formatDateTime(new Date()).substr(0, 10);
			
			// this.nowadays = '2025-01-19';
			// console.log(this.current);
			this.tickWeek = -1;
			if (this.current !== 1) {
				return;
			}
			var arr = this.week_list[this.to_week_index];
			for (var i = 0; i < arr.length; i++) {
				var e = arr[i];
				if (e.date.replace(/-(\d)(?!\d)/g, '-0$1') === this.nowadays) {
					this.tickWeek = i;
				}
			}
		},
	...
	&.brief {
		.box {
			...
			.clue {
				...
				.year {
					margin-bottom: 24rpx; // 14rpx;
				}
				.month {
					font-size: 50rpx; // 46rpx;
					line-height: 50rpx;
