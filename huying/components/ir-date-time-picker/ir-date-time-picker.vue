	...
	<view>
		...
		<view v-if="visible" class="picker-con" :style="{background: pickerBg}">
			...
			<picker-view
				...
				<ir-date-time-picker-column :list-data="minutes" :index-current="indexMin < 0 ? pickerVal[4] : indexMin" :index-column="4" :format="'%%val%%分'" @change="handleColumn"></ir-date-time-picker-column>
				<!-- <picker-view-column>
					<view class="item"
...
<script>
	import irDateTimePickerColumn from './ir-date-time-picker-column.vue';
	
	export default {
		name: "IrDateTimePicker",
		components: {
		    irDateTimePickerColumn
		},
		props: {
			start: {
				type: [String, Number],
		...
		methods: {
			...
			confirm() {
				...
				else if (this.indexMin >= this.minutes.length) {
					this.indexMin = this.minutes.length ? 0 : -1;
				}
				const t = `${trimN(this.hours[val[3]])}:${trimN(this.indexMin < 0 ? 0 : this.minutes[this.indexMin])}`
				const date = `${d} ${t}`
				this.date = date
				let rV = date
			...
			setStartEnd() {
				...
				this.startObj.days = this.startObj.days.filter(e => e >= sd);
			...
			initCurr(val, month, day) {
				...
				if (this.abstractMode) {
					var arr = this.minutes.filter(e => e % 5 === 0);
					if (arr.length) {
						this.minutes = arr;
					}
			...
			handleColumn(e) {
				// console.log(e.currentIndex + ' - ' + e.currentColumn);
				if (e.currentColumn === 4) {
					this.indexMin = e.currentIndex;
				}
			}
		}
	}
